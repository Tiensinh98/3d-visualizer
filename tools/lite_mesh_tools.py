...
def get_boundary_sedges(surface_exo_data):
    sides = get_sides(surface_exo_data)
    sedges = get_sedges(surface_exo_data, sides)
    boundary_sedges = sedges['mesh_node_idxs']
...
if __name__ == '__main__':
    import numpy as np
    exo_file = r"E:\Research\learn_qt\learn_OpenGL\exo_files\global_solution_000_component_1_slice.exo"
    ...
    # new_exo_data.write(exo_file.replace('.exo', '_surface.exo'))
    exit(0)
    ...
    vertices = new_exo_data.coords
    indices = new_exo_data.exo_block_datas[0].elems
    edge_indices = boundary_sedges
    scalar_values = new_exo_data.node_field_values['von_mises'][0]
    for i in range(29, 41):
        np.savetxt(fr'E:\Research\Udemy\web-development\session-20-apis\WeatherProject\mesh\component_{i}\indices.txt', indices, fmt='%d')
        np.savetxt(fr'E:\Research\Udemy\web-development\session-20-apis\WeatherProject\mesh\component_{i}\edge_indices.txt', edge_indices, fmt='%d')
        np.savetxt(fr'E:\Research\Udemy\web-development\session-20-apis\WeatherProject\mesh\component_{i}\vertices.txt', exo_data.coords + i - 1, fmt='%.3e')
        np.savetxt(fr'E:\Research\Udemy\web-development\session-20-apis\WeatherProject\field_values\component_{i}\von_mises.txt', scalar_values, fmt='%.3e')
        np.savetxt(fr'E:\Research\Udemy\web-development\session-20-apis\WeatherProject\field_values\component_{i}\stress_xx.txt', new_exo_data.node_field_values['stress_xx'][0], fmt='%.3e')
    # for i in range(1, 26):
    #     textContent = ''
    #     textContent += '#vertices\n'
    #     vertices = vertices + i
    #     for vertex in vertices:
    #         textContent += 'v ' + list_to_string(vertex) + '\n'
    #     textContent += '#indices\n'
    #     for index in indices:
    #         textContent += 'i ' + list_to_string(index) + '\n'
    #     textContent += '#edge indices\n'
    #     for edgeIndex in edge_indices:
    #         textContent += 'ei ' + list_to_string(edgeIndex) + '\n'
    #     textContent += '#scalar values\n'
    #     for scalarValue in scalar_values:
    #         textContent += 'sv ' + str(scalarValue) + '\n'
    #
    #     # Write the text content to a file
    #     filepath = f'E:/Research/Udemy/web-development/session-20-apis/WeatherProject/geometry_{i}.txt'
